...
function updateCurrentDate() {
    ...
    // Calculate which Sunday of the month we're on
    const firstDay = new Date(now.getFullYear(), now.getMonth(), 1).getDay();
    const sundayOfMonth = Math.floor((currentDate + firstDay - 1) / 7);
    
    setTimeout(() => {
        document.querySelectorAll('.sunday-date').forEach(dateElement => {
            if (dateElement.textContent.includes(currentDate.toString())) {
                dateElement.style.backgroundColor = 'var(--secondary-color)';
            }
        });
    }, 100);
}
